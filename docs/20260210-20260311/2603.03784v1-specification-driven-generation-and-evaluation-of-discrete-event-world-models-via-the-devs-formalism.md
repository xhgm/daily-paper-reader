...
tags: ["query:q13"]
score: 9.0
evidence: 用于智能体系统的离散事件世界模型
tldr: 本研究针对智能体系统中世界模型在一致性与灵活性之间的权衡问题，提出了一种基于DEVS形式化方法的离散事件世界模型生成框架。该方法利用大语言模型（LLM）从自然语言规范中直接合成可执行模型，通过分阶段流水线分离结构推理与逻辑实现。通过结构化事件追踪验证模型的一致性与可靠性，为在线执行提供了可验证、高效且一致的长时程模拟方案。
selection_source: fresh_fetch
motivation: 旨在解决手工模拟器成本高昂与神经模型难以验证之间的矛盾，寻求一种既具可靠性又具灵活性的世界模型构建方法。
method: 采用DEVS形式化方法，利用分阶段的LLM生成流水线将系统结构推理与组件级事件逻辑分离，从自然语言规范合成可执行模型。
result: 成功生成了能够产生结构化事件追踪的世界模型，并能通过规范衍生的时序和语义约束进行自动化验证与诊断。
conclusion: 该方法为离散事件环境提供了一种一致、可验证且支持在线按需合成的高效世界模型构建方案。
---

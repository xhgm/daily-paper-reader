...
tags: ["query:q18"]
score: 10.0
evidence: 用于密集杂乱环境下双臂操作的分层VLA框架
tldr: 针对高密度杂乱环境下的双臂操作难题，本文提出HSC-VLA分层框架。该框架将高层视觉语义推理与底层运动执行解耦，通过“大脑”模块生成任务相关的场景掩码以过滤干扰，并由基于扩散策略的“小脑”模块执行操作。实验表明，该方法在超市货架等复杂场景中显著优于端到端模型，大幅提升了长程任务的成功率和鲁棒性。
selection_source: fresh_fetch
motivation: 现有的视觉-语言-动作模型在密集杂乱环境中容易受到无关视觉干扰的影响，导致指令遵循失败和性能下降。
method: 提出一种分层架构，利用高层模块生成场景掩码过滤干扰，并结合底层扩散策略进行双臂协同操作。
result: "在密集杂乱的超市货架实验中，HSC-VLA达到了86.7%的成功率，比最强的端到端基准模型高出52.4%。"
conclusion: 通过显式的场景清理抽象和分层解耦，可以有效提升机器人处理复杂长程任务的鲁棒性和故障恢复能力。
---

## 摘要
现代视觉-语言-动作（VLA）模型在稠密操作环境中经常面临严重的指令遵循失败问题。在这些环境中，与任务无关的视觉杂物会分散注意力，破坏定位关联，并在复杂的长程场景中显著降低性能。为了克服单体式端到端架构的表示瓶颈，我们提出了 HSC-VLA，这是一个分层框架，通过显式的场景清理抽象，将高层视觉语义推理与低层高频感觉运动执行解耦。HSC-VLA 采用一个高层“大脑”（Brain）来分解长程任务，并生成特定于任务的场景掩码，在抑制干扰物的同时保留与任务相关的几何信息。过滤后的观测结果随后被传递给低层“小脑”（Cerebellum），这是一个基于扩散的策略，仅利用掩码过滤后的视觉和本体感受来执行双臂操作。在稠密杂乱的超市货架上进行的广泛实验表明，HSC-VLA 在高密度杂乱环境下实现了 86.7% 的综合成功率，比表现最好的单体基准模型（π0-Full FT，34.3%）高出 52.4%。HSC-VLA 还表现出强大的长程任务性能，在杂物分类任务中达到 72%，在补货任务中达到 66%，证明了其在复杂杂乱操作中强大的鲁棒性和有效的故障恢复能力。

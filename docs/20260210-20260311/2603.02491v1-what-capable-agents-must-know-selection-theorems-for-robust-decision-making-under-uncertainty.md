...
tags: ["query:q22"]
score: 9.0
evidence: 智能体中世界模型和预测性内部状态的选择定理
tldr: 本研究探讨了高效智能体在不确定环境下决策时，其内部结构是否必须包含世界模型。通过证明“选择定理”，研究指出在预测任务中实现低平均遗憾的智能体，必然在其内部实现了预测性的结构化状态。该理论涵盖了随机策略和部分可观测环境，证明了高性能表现与内部预测模型之间的必然联系。
selection_source: fresh_fetch
motivation: 传统理论仅证明了世界模型可以实现最优控制，但未证明其对于高效决策是否具有必要性。
method: 将预测建模简化为二元博弈决策，通过遗憾界限限制次优决策的概率，从而推导出内部状态的结构要求。
result: 证明了低遗憾智能体在全观测下能恢复转移核，在部分观测下必须具备类似信念的记忆和预测状态。
conclusion: 智能体若要在复杂任务中表现出色，其内部必然会演化出能够反映环境动态的预测性表征。
---

## 摘要
随着人工智能体变得越来越强大，智能体在不确定性下胜任工作所必需的内部结构是什么？经典结果表明，最优控制可以利用信念状态（belief states）或世界模型来实现，但并未证明这些表示是必需的。我们证明了定量的“选择定理”（selection theorems），表明在结构化的动作条件预测任务族上，较低的平均情况悔值（average-case regret）会迫使智能体实现一种预测性的、结构化的内部状态。我们的结果涵盖了随机策略、部分可观测性以及任务分布下的评估，且无需假设最优性、确定性或对显式模型的访问。在技术层面上，我们将预测建模简化为二元“下注”决策，并证明悔值界限限制了次优下注的概率质量，从而强制执行分离高边际结果所需的预测区分。在完全可观测的设置下，这产生了干预转移核（interventional transition kernel）的近似恢复；在部分可观测性下，这暗示了类信念记忆和预测状态的必要性，解决了先前世界模型恢复工作中的一个开放性问题。

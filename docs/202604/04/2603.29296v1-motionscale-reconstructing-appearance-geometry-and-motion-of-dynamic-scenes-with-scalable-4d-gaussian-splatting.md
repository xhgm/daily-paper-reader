...
motivation: 现有方法在处理复杂动态场景的单目视频时，难以兼顾准确的 3D 几何恢复和长期的运动一致性。
method: 核心是采用以簇为中心的基变换来参数化可扩展运动场，并结合背景扩展与前景传播的解耦渐进式优化策略。
result: 在多个真实世界挑战性数据集上的实验表明，该方法在重建质量和时间稳定性上均显著超越了当前最先进的技术。
conclusion: MotionScale 通过自适应运动建模和分阶段优化，为大规模长序列动态场景的高效 4D 重建提供了强有力的技术支撑。
---

...
title_zh: RoboRouter：面向机器人操纵的无需训练的策略路由
...
tags: ["query:q9"]
score: 9.0
evidence: 利用VLA和VA模型进行机器人操作的策略路由
...
## 摘要
机器人操纵研究已经发展出多样化的策略范式，包括视觉-语言-动作（VLA）模型、视觉-动作（VA）策略以及基于代码的组合方法。具体策略通常在特定任务分布上获得高成功率，但在分布之外的泛化能力有限。与其提出另一种单一策略，我们建议通过智能策略路由来利用现有方法的互补优势。我们推出了 RoboRouter，这是一个无需训练的框架，它维护一个异构策略池，并通过积累的执行经验学习为每个任务选择表现最佳的策略。给定一个新任务，RoboRouter 构建语义任务表示，检索相似任务的历史记录，在无需试错的情况下预测最优策略选择，并结合结构化反馈来优化后续的路由决策。将新策略集成到系统中仅需轻量级评估，且不产生训练开销。在仿真基准测试和真实世界评估中，RoboRouter 的表现始终优于单个策略，在仿真中平均成功率提高了 3% 以上，在真实世界设置中提高了 13% 以上，同时保持了执行效率。我们的结果表明，在异构的现成策略之间进行智能路由，为构建功能更强大的机器人系统提供了一条实用且可扩展的途径。
